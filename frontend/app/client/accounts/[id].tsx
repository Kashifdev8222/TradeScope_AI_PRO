...
export default function AccountDetailScreen() {
  ...
  const fetch = async () => { if (!id) return; setLoading(true); try { setDetail(await accountApi.getDetail(id)); } catch {} finally { setLoading(false); } };
  ...
  const updateSettings = async (key: string, value: any) => {
    ...
    setSaving(true);
    try {
      await accountApi.updateSettings(id, { [key]: value });
      fetch();
  ...
  if (!detail) return <ScreenContainer max={1400}><View style={s.ctr}><Text style={{ color: colors.danger }}>Not found</Text></View></ScreenContainer>;
  ...
  return (
    <ScreenContainer max={1400} scroll>
      ...
      {/* Trading Settings — editable toggles */}
      <View style={s.card}>
        <Text style={s.cardTitle}>Trading Settings</Text>
        <ToggleRow
          icon="flash-outline" label="One-Click Trading"
          value={settings?.one_click_enabled || false}
          onToggle={(v) => updateSettings("one_click_enabled", v)}
        />
        <View style={[s.row, s.rowBorder]}>
          <Ionicons name="layers-outline" size={16} color={colors.textMuted} />
          <Text style={s.rowLabel}>Default Order Size</Text>
          <TextInput
            style={s.numInput}
            value={String(settings?.default_order_size || "0.01")}
            keyboardType="numeric"
            onSubmitEditing={(e) => updateSettings("default_order_size", parseFloat(e.nativeEvent.text) || 0.01)}
            placeholder="0.01"
          />
        </View>
        <ToggleRow
          icon="shield-outline" label="Manual AI Approval"
          value={settings?.manual_ai_approval || false}
          onToggle={(v) => updateSettings("manual_ai_approval", v)}
        />
      </View>
...
function Row({ icon, label, value, badge, color, last }: any) {
  return (
    <View style={[s.row, !last && s.rowBorder]}>
      ...
      {badge ? (
        <View style={[s.badge, { backgroundColor: color + "18" }]}><View style={[s.badgeDot, { backgroundColor: color }]} /><Text style={[s.rowValue, { color }]}>{value}</Text></View>
      ) : <Text style={s.rowValue}>{value}</Text>}
    </View>
  );
}

function ToggleRow({ icon, label, value, onToggle }: any) {
  return (
    <View style={[s.row, s.rowBorder]}>
      <Ionicons name={icon} size={16} color={colors.textMuted} />
      <Text style={s.rowLabel}>{label}</Text>
      <Switch value={value} onValueChange={onToggle} trackColor={{ false: colors.inputBorder, true: colors.accent }} thumbColor="#fff" />
    </View>
  );
}
...
function Block({ label, value }: any) {
  return (
    <View style={s.block}><Text style={s.blockLabel}>{label}</Text><Text style={s.blockValue}>{value}</Text></View>
  );
}
...
const s = StyleSheet.create({
  ...
  blocks: { flexDirection: "row", flexWrap: "wrap", gap: 14 },
